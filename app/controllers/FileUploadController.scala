  ...
  def get = authAction.async {
    implicit request =>
      for {
        ...
        Ok(upscan_csv_file_upload(response, formPartialRetriever))
      }
  }
  ...
  def showResult() = authAction.async { implicit request =>
    ...
    for {
      uploadResult <- sessionService.getCallbackRecord
          ...
          uploadResult match {
            case Some(result: UploadStatus) => Ok(views.html.upload_result(result, formPartialRetriever))
  ...
  def failure(errorCode: String, errorMessage: String, errorRequestId: String) = authAction {
    implicit request =>
          Ok(views.html.failure(Messages("gmp.bulk.failure.generic"),
            ...
            Messages("gmp.bulk_failure_generic.title"), formPartialRetriever))
  }
