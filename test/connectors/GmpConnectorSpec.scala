  ...
  "The GMP Connector" must {
    ...
    "performing a single calculation" must {
      ...
      "return a calculation response" in {
        implicit val user = AuthContext(authority = Authority("1234", Accounts(gmp = Some(GmpAccount("link", PsaId(psaId)))), None, None, CredentialStrength.None, ConfidenceLevel.L50))
      ...
      "throw an exception with invalid user" in {
        ...
        val nino = RandomNino.generate
        implicit val user = AuthContext(authority = Authority("1234", Accounts(gmp = None), None, None, CredentialStrength.None, ConfidenceLevel.L200))
