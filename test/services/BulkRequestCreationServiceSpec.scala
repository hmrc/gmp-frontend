...
class BulkRequestCreationServiceSpec extends PlaySpec with ScalaFutures with MockitoSugar with GuiceOneServerPerSuite {
  ...
  "Bulk Request Creation Service" must {
    ...
    "return Bulk Request 1" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("1"), bulkRequest1.email, bulkRequest1.reference).toOption.get
    ...
    "return Bulk Request 2" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("2"), bulkRequest2.email, bulkRequest2.reference).toOption.get

      bulkRequest.getFirstValid.surname mustBe "O'BRIEN"
    }

    "return Bulk Request with multiple calcs" in {
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("3"), bulkRequest3.email, bulkRequest3.reference).toOption.get
    ...
    "contain Nones when dates are left blank" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("4"), bulkRequest4.email, bulkRequest4.reference).toOption.get
    ...
    "termination date for person who is still in scheme should be same as reval date" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("11"), bulkRequest11.email, bulkRequest11.reference).toOption.get
    ...
    "termination date for person with blank input termination date should be None" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("10"), bulkRequest10.email, bulkRequest10.reference).toOption.get
    ...
    "termination date for person who left scheme post-2016 should be correct date entered" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("12"), bulkRequest12.email, bulkRequest12.reference).toOption.get
    ...
    "contain Nones for other options" in {
      ...
      val bulkRequest: BulkCalculationRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("5"), bulkRequest4.email, bulkRequest4.reference).toOption.get
    ...
    "return bulk requests with field validation messages" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("7"), invalidBulkRequest.email, invalidBulkRequest.reference).toOption.get
    ...
    "cope with invalid number and date" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("15"), invalidBulkRequest.email, invalidBulkRequest.reference).toOption.get
      val errors = bulkRequest.calculationRequests.head.validationErrors
    ...
    "return bulk requests with general line error" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("13"), invalidBulkRequest.email, invalidBulkRequest.reference).toOption.get
      val errors = bulkRequest.calculationRequests.head.validationErrors
    ...
    "return bulk request with dual calc Yes" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("8"), anotherBulkRequest.email, anotherBulkRequest.reference).toOption.get
    ...
    "trim spaces around fields" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("14"), bulkRequest14.email, bulkRequest14.reference).toOption.get
    ...
    "cope with termination date with wrong format dd MM yyyy with no slashes" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("16"), bulkRequest16.email, bulkRequest16.reference).toOption.get
      bulkRequest.getFirstValid.firstForename mustBe "TIM"
    }

    "convert scon to uppercase" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("17"), bulkRequest17.email, bulkRequest17.reference).toOption.get
    ...
    "not send the revaluation date if provided when calculation type is leaving" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("18"), bulkRequest18.email, bulkRequest18.reference).toOption.get
      bulkRequest.getFirstValid.revaluationDate mustBe None
      bulkRequest.getFirstValid.revaluationRate mustBe None
    }

    "not send the revaluation date if provided when calculation type is PA" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("20"), bulkRequest20.email, bulkRequest20.reference).toOption.get
      bulkRequest.getFirstValid.revaluationDate mustBe None
    }

    "not send the revaluation date if provided when calculation type is SPA" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("21"), bulkRequest21.email, bulkRequest21.reference).toOption.get
      bulkRequest.getFirstValid.revaluationDate mustBe None
    }

    "not send the termination date if SM is provided, when calculation type is survivor" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("24"), bulkRequest24.email, bulkRequest24.reference).toOption.get
      bulkRequest.getFirstValid.terminationDate mustBe None
    }
    ...
    "not send dual calc when calculation type is survivor" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("19"), bulkRequest19.email, bulkRequest19.reference).toOption.get
      bulkRequest.getFirstValid.dualCalc mustBe 0
    }

    "send term date when term date is on or after 06/04/2016" in {
      val bulkRequest = TestBulkRequestCreationService.createBulkRequest(upscanCallback("22"), bulkRequest22.email, bulkRequest22.reference).toOption.get
  ...
  def lineListFromCalculationRequestLine(line: BulkCalculationRequestLine): List[Char] = {
...

    {
      (l collect {
        case (None, i) => ""
        case (Some(s: String), i) if s.matches(dateRegEx.regex) => LocalDate.parse(s).format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))
