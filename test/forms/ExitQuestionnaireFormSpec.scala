...
class ExitQuestionnaireFormSpec extends PlaySpec with MockitoSugar {
  "Exit Questionnaire Form" must {
    "return no errors with no optional input" in {
      ...
      val exitQuestionnaireFormResults = exitQuestionnaireForm.bind(Json.toJson(ExitQuestionnaire(None,None, None)))
    ...
    "return no errors with valid input" in {
      ...
      val exitQuestionnaireFormResults = exitQuestionnaireForm.bind(Json.toJson(ExitQuestionnaire(Some(ExitQuestionnaire.VERY_EASY), Some(ExitQuestionnaire.VERY_SATISFIED), Some("These are the comments"))))
