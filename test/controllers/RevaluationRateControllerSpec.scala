...
class RevaluationRateControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar {
  ...
  "Revaluation Rate controller GET " must {
    ...
    "authenticated users" must {
      ...
      "include the correct header when Payable Date scenario selected" in {
        when(mockSessionService.fetchGmpSession()(Matchers.any())).thenReturn(Future.successful(Some(gmpSession.copy(scenario = CalculationType.PAYABLE_AGE))))
        val result = TestRevaluationRateController.get(FakeRequest())
            contentAsString(result) must include(Messages("gmp.revaluation_rate.header"))
      }
  ...
  "Revaluation controller POST" must {
    ...
    when(mockSessionService.fetchScenario()(Matchers.any())).thenReturn(Future.successful(None))
    ...
    "authenticated users" must {
      ...
      "with valid data" must {
        ...
        "redirect" in {
          ...
          when(mockSessionService.cacheRevaluationRate(Matchers.any())(Matchers.any())).thenReturn(Future.successful(Some(gmpSession)))
            val postData = Json.toJson(
        ...
        "respond with error when rate not stored" in {
          when(mockSessionService.cacheRevaluationRate(Matchers.any())(Matchers.any())).thenReturn(Future.successful(None))
            val postData = Json.toJson(
              RevaluationRate(Some("hmrc"))
            )
            intercept[RuntimeException] {
