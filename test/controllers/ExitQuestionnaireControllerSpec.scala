    ...
    "authenticated users" must {
      ...
      "with valid data" must {
        ...
        "redirect to thank you page" in {
          ...
          withAuthorisedUser { request =>
            val postData = Json.toJson(
              ExitQuestionnaire(Some(ExitQuestionnaire.VERY_EASY), Some(ExitQuestionnaire.VERY_SATISFIED), Some("These are the comments"), Some("Full Name"), Some("email@address.com"), Some("0123456789"))
            )
            val result = TestExitQuestionnaireController.post.apply(request.withJsonBody(postData))
            status(result) must equal(SEE_OTHER)
          }
